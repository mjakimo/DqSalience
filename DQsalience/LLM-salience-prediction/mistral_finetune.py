...
def load_and_format_data():
    data_dir = "/home/mhao/qsalience_project/data"
    json_files = glob.glob(os.path.join(data_dir, "prepared_*.json"))
    
    if not json_files:
        raise ValueError(f"no json file under {data_dir} ")
            ...
            for item in items:
                try:
                    ...
                    if context and question and label:
                        text = (
                            f"<s>[INST] {SCORING_RUBRIC}\n\n"
                            f"### Context:\n{context}\n\n"
                            f"### Question to Evaluate:\n{question}\n\n"
                            f"What is the salience score (0-8)? [/INST] {label}</s>"
                        )
...
training_args = TrainingArguments(
    output_dir="./mistral_salience_lora",
...
trainer.model.save_pretrained("./mistral_salience_lora_final")
tokenizer.save_pretrained("./mistral_salience_lora_final")