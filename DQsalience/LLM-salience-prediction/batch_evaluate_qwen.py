    ...
    with open(json_file, 'r', encoding='utf-8') as f:
        ...
        for item in items:
            ...
            if not gold_label.isdigit():
                continue
            all_valid_items.append({"context": context, "question": question, "gold_label": int(gold_label)})
...
adapter_path = "./qwen_native_lora_final"

print("Loading Qwen Native model...")
...
results = []
for item in tqdm(test_items, desc="Evaluating Qwen"):
    prompt = (
        f"<|im_start|>user\n"
        f"{SCORING_RUBRIC}\n\n"
        f"### Context:\n{item['context']}\n\n"
        f"### Question to Evaluate:\n{item['question']}\n\n"
        f"What is the salience score (0-8)?<|im_end|>\n"
...
df.to_csv("pure_test_results_qwen_native.csv", index=False, encoding='utf-8-sig')